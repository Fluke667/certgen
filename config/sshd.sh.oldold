...
set_hostkeys() {
    printf '%s\n' \
        'set /etc/ssh/sshd_config/HostKey[1] /etc/ssh/keys/ssh_host_rsa_key' \
        'set /etc/ssh/sshd_config/HostKey[2] /etc/ssh/keys/ssh_host_dsa_key' \
        'set /etc/ssh/sshd_config/HostKey[3] /etc/ssh/keys/ssh_host_ecdsa_key' \
        'set /etc/ssh/sshd_config/HostKey[4] /etc/ssh/keys/ssh_host_ed25519_key' \
    | augtool -s
}
...
if [[ "${SFTP_MODE}" == "true" ]]; then
    ...
    printf '%s\n' \
        'set /etc/ssh/sshd_config/Subsystem/sftp "internal-sftp"' \
        'set /etc/ssh/sshd_config/AllowTCPForwarding no' \
        'set /etc/ssh/sshd_config/X11Forwarding no' \
        'set /etc/ssh/sshd_config/ForceCommand internal-sftp' \
        "set /etc/ssh/sshd_config/ChrootDirectory ${SFTP_CHROOT}" \
    | augtool -s
fi

# Enable GatewayPorts
if [[ "${GATEWAY_PORTS}" == "true" ]]; then
    echo 'set /etc/ssh/sshd_config/GatewayPorts yes' | augtool -s
fi
